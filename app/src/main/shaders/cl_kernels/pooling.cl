...
//fix clspv _Z3minll
long __attribute__((overloadable)) min(long v1, long v2) {
  return v1 < v2 ? v1 : v2;
}
//fix clspv _Z3minll
long __attribute__((overloadable)) max(long v1, long v2) {
  return v1 > v2 ? v1 : v2;
}
...
__kernel void TEMPLATE(sto_pool_backward,Dtype)(
  ...
  for (int_tp index = get_global_id(0); index < nthreads; index +=
    ...
    const int_tp pwend = min(w / stride_w + 1, pooled_width);
    Dtype gradient = 0.0;
    const int_tp ncc = n * channels + c;
    __global const Dtype* rand_idx_slice = rand_idx
        + ncc * pooled_height * pooled_width;
    __global const Dtype* top_diff_slice = top_diff
        + ncc * pooled_height * pooled_width;
    for (int_tp ph = phstart; ph < phend; ++ph) {
      for (int_tp pw = pwstart; pw < pwend; ++pw) {
        const int_tp ppp = ph * pooled_width + pw;
        gradient += index == (int_tp) (rand_idx_slice[ppp]) ? top_diff_slice[ppp] : 0;
      }
    }
    bottom_diff[index] = gradient;
  }
