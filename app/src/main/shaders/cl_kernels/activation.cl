...
__kernel void TEMPLATE(relu_backward,Dtype)(const int_tp n,
                                            ...
                                            Dtype negative_slope) {
  for (int_tp index = get_global_id(0); index < n; index += get_global_size(0)) {
    //out_diff[index] = in_diff[index]  * ((in_data[index] > 0?1.0:0.0) + (in_data[index] <= 0?1.0:0.0) * negative_slope);
    out_diff[index] = in_data[index] > 0 ? in_diff[index] : in_diff[index] * negative_slope;
  }
}
...
__kernel void TEMPLATE(sigmoid_forward,Dtype)(const int_tp n,
  ...
  for (int_tp index = get_global_id(0); index < n; index += get_global_size(0)) {
      ...
      Dtype base = 1.0;
    out[index] = base / (base + exp(-in[index]));
  }
}
...
__kernel void TEMPLATE(prelu_param_backward,Dtype)(const int_tp n, const int_tp rows,
  ...
  for (int_tp index = get_global_id(0); index < n; index += get_global_size(0)) {
    //out_diff[index] = in_diff[index] * in_data[index] * (in_data[index] <= 0?1.0:0.0);
      out_diff[index] = in_data[index] > 0 ? 0 : in_diff[index] * in_data[index];
    for (int k = 1; k < rows; k++) {
      int_tp ca_index = index + k * rowPitch;
      out_diff[index] += in_data[ca_index] > 0 ? 0 : in_diff[ca_index] * in_data[ca_index];
    }
  }
}
